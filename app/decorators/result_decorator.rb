class ResultDecorator < Draper::Decorator
  ...
  def fastest_duration_icon
    if result.fastest_duration
      "#{icon('trophy')} #{icon('clock-o', duration_in_words(result.fastest_duration))} (#{previous_time_difference(:fastest_duration)})".html_safe
    end
  end

  def median_duration_icon
    if result.median_duration
      "σ #{icon('clock-o', duration_in_words(result.median_duration))} (#{previous_time_difference(:median_duration)})".html_safe
    end
  end
